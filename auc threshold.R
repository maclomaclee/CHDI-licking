...
colnames(observed) <- c("mouse", "genotype", "sex", "training_type", "training_session", "AUC1","AUC2")
  ...
  ifelse (observed[i,6] > 0.8,observed[i,6] <- 1, observed[i,6] <- 0)
}
...
ggplot(data = m_e_h_epoch, mapping = aes(x = session, y = AUC, group = type))  + geom_line(aes(color=type)) + labs(title = 0.8)
